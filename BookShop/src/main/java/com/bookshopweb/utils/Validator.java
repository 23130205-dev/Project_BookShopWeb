...
public class Validator<T> {
    ...
    public Validator<T> isNotNull() {
        Optional<String> violation = valueValidator != null ? Optional.empty() : Optional.of("Không để trống");
        violation.ifPresent(violations::add);
        return this;
    }

    public Validator<T> isNotNullAndEmpty() {
        Optional<String> violation = valueValidator != null && !String.valueOf(valueValidator).trim().isEmpty() ? Optional.empty() : Optional.of("Không để trống");
        violation.ifPresent(violations::add);
        return this;
    }

    public Validator<T> isNotEmpty() {
        Optional<String> violation = Optional.ofNullable(valueValidator)
    ...
    public Validator<T> isSmallerThan(double number, String subjectName) {
        Optional<String> violation = Optional.ofNullable(valueValidator)
                .filter(obj -> !(Double.parseDouble(String.valueOf(valueValidator)) <= number))
